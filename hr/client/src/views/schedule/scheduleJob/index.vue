...
<el-form  ref='queryForm' :model='queryFormData' label-width='100px' v-if='!isQueryConditionPanel' class='dc-el-form_ElQueryForm'>
      <el-row>
        <el-col :span="6">
      <el-form-item prop="name"  label='任务名称' class='dc-el-form-item_SingleInput'>
      <el-input  v-model='queryFormData.name' :maxLength='128' placeholder='请输入任务名称' clearable class='dc-el-input_SingleInput'> </el-input>
    </el-form-item>
    </el-col>
      </el-row>
    </el-form>
<el-row  gutter='0' type='flex' justify='end' align='top' class='dc-el-row_ElRow'><el-tooltip  v-show='permission.add && currentUser.id == 1000' disabled class='dc-el-tooltip_Button'><el-button  type='primary' icon='el-icon-plus' v-on:click='onCreate'>添加</el-button></el-tooltip></el-row>
...
<ux-table-column :field="tableColumns['132'].field"
      ...
      :params="{sortId: '132'}" tree-node resizable min-width='180px' align='left' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'>
                <template slot-scope='{row, rowIndex}'>
                  <div v-html='((cellValue, row, index) => { switch (cellValue) {case "1": return "运行中";case "0": return "已暂停"; default: return "";} })(row.status, row, rowIndex)'></div>
                </template>
             </ux-table-column>
<ux-table-column :field="tableColumns['134'].field"
      ...
      :params="{sortId: '134'}" tree-node resizable min-width='160px' align='left' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'></ux-table-column>
<ux-table-column :field="tableColumns['138'].field"
      ...
      :params="{sortId: '138'}" tree-node resizable min-width='160px' align='right' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'></ux-table-column>
<ux-table-column :field="tableColumns['133'].field"
      ...
      :params="{sortId: '133'}" tree-node resizable min-width='180px' align='left' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'></ux-table-column>
<ux-table-column :field="tableColumns['135'].field"
      ...
      :params="{sortId: '135'}" tree-node resizable min-width='180px' align='left' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'></ux-table-column>
<ux-table-column :field="tableColumns['136'].field"
      ...
      :params="{sortId: '136'}" tree-node resizable min-width='160px' align='right' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'></ux-table-column>
<ux-table-column :field="tableColumns['137'].field"
      ...
      :params="{sortId: '137'}" tree-node resizable min-width='180px' align='left' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'></ux-table-column>
<ux-table-column :field="tableColumns['139'].field"
      ...
      :params="{sortId: '139'}" tree-node resizable min-width='180px' align='left' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'></ux-table-column>
<ux-table-column :field="tableColumns['140'].field"
      ...
      :params="{sortId: '140'}" tree-node resizable min-width='180px' align='left' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'></ux-table-column>
<ux-table-column :field="tableColumns['141'].field"
      ...
      :params="{sortId: '141'}" tree-node resizable min-width='180px' align='left' header-align='center' sortable show-overflow class='dc-ux-table-column_ElTableColumn'></ux-table-column>
<ux-table-column title='操作' :params="{sortId: '114'}"  tree-node resizable width='160px' min-width='160px' fixed='right' align='left' header-align='center' class='dc-ux-table-column_ElTableOptColumn'>
...
<OperationIcon  v-on:click='onExport(tableData)' type='primary' v-show='permission.export' content='导出excel文件' placement='top' icon-name='el-icon-download' class='dc-OperationIcon_IconButton'></OperationIcon>
                  ...
                  <OperationIcon  v-on:click='onStatusScheduleJob(scope.$index, scope.row)' type='primary' v-show='scope.row.status == "1"' content='暂停' placement='bottom' icon-name='el-icon-video-pause' class='dc-OperationIcon_IconButton'></OperationIcon>
<OperationIcon  v-on:click='onStatusScheduleJob(scope.$index, scope.row)' type='primary' v-show='scope.row.status == "0"' content='运行' placement='bottom' icon-name='el-icon-video-play' class='dc-OperationIcon_IconButton'></OperationIcon>
...
<OperationIcon  v-on:click='onDelete(scope)' type='danger' v-show='permission.remove && currentUser.id == 1000' content='删除' placement='top' icon-name='el-icon-delete' class='dc-OperationIcon_IconButton'></OperationIcon>
...
import { listScheduleJobPage, deleteScheduleJob,endTask,startTask } from '@/api/schedule/scheduleJob'
...
tableDataTotal: 0,
      ...
      searchList: [],
      tableId: '1295275790451941378',
      schemeId: '1295275790451941376',
    }
  },
  computed: {},
  methods: {
    onStatusScheduleJob(index, row) {
      if (row.status === "1") {
        endTask({ id: row.id })
          .then((responseData) => {
            if (responseData.code == 100) {
              ...
              this.$message.error(responseData.data);
            }
          })
          .catch((error) => {
  ...
  getList() {
    ...
    async pageInit() {
      ...
      try {
        ...
        if(listScheduleJobRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listScheduleJobRespData.data.total
          this.tableData = listScheduleJobRespData.data.rows ? listScheduleJobRespData.data.rows :[]
    ...
    onDelete(scope) {
      this.$confirm('确定删除吗？', '确认', {
        ...
        this.setLoad()
        deleteScheduleJob(scope.row).then(responseData => {
          if(responseData.code == 100) {
            this.getList()
