...
export default {
  ...
  methods: {
      ...
      this.search.params = [
        {
          columnName: 'empolyno',
          queryType: '=',
          value: !validatenull(this.checkUser.empolyno) ? this.checkUser.empolyno : null
        }
      ]
    ...
    initUser(){
      getUserById(currentUser.id).then(resp => {
        if(resp.code == 100){
          this.checkUser = resp.data
        }
      })
    },
    pageInit() {
      //this.setLoad()
      this.$refs.editForm.dialogProps.visible = true
      // try {
      ...
      //       columnName: 'empolyno',
      //       queryType: '=',
      //       value: this.checkUser.empolyno
      //     }
      //   ]
      //   this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
      //   listHrmPayrollBillDetailsPage(this.search).then(resp => {
      //     if(resp.code == 100){
      //       this.tableDataTotal = resp.data.total
      //       this.tableData = resp.data.rows ? resp.data.rows : []
      //     }
      //   })
      //   listResourcePermission(this.$route.meta.routerId).then(resp => {
      //     if(resp.code == 100){
      //       this.permission.view = resp.data.find((item) => {
      //         return item.permission === 'hrmPayrollBillDetails:read'
      //       })
      //       this.permission.export = resp.data.find((item) => {
      //         return item.permission === 'hrmPayrollBillDetails:export'
      //       })
      //       this.permission.add = resp.data.find((item) => {
      //         return item.permission === 'hrmPayrollBillDetails:create'
      //       })
      //       this.permission.edit = resp.data.find((item) => {
      //         return item.permission === 'hrmPayrollBillDetails:update'
      //       })
      //       this.permission.remove = resp.data.find((item) => {
