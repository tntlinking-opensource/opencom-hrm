<template>
  <el-row class="dc-container" v-loading="loading">
    ...
    >
      <el-row>
        <el-col :span="6">
          <el-form-item prop="name" label="类型" class="dc-el-form-item_SingleInput">
            <el-input
...
import { listHrmLaidTypePage, deleteHrmLaidType } from '@/api/hrm/hrmLaidType'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [],
      tableId: '1457254743943433140',
      schemeId: '1457254743943433138'
    }
  },
  computed: {},
  methods: {
    getList() {
      ...
      listHrmLaidTypePage(this.search)
        .then((responseData) => {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 离职类型hrm_laid_type
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listHrmLaidTypeRespData, listPermissionRespData] = await Promise.all([
          listHrmLaidTypePage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listHrmLaidTypeRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listHrmLaidTypeRespData.data.total
          this.tableData = listHrmLaidTypeRespData.data.rows ? listHrmLaidTypeRespData.data.rows : []
          this.permission.view = listPermissionRespData.data.find((item) => {
            return item.permission === 'hrmLaidType:read'
          })
          this.permission.export = listPermissionRespData.data.find((item) => {
            return item.permission === 'hrmLaidType:export'
          })

          this.permission.add = listPermissionRespData.data.find((item) => {
            return item.permission === 'hrmLaidType:create'
          })
          this.permission.edit = listPermissionRespData.data.find((item) => {
            return item.permission === 'hrmLaidType:update'
          })
          this.permission.remove = listPermissionRespData.data.find((item) => {
            return item.permission === 'hrmLaidType:delete'
          })
        } else {
          this.showMessage(listPermissionRespData.code != 100 ? listPermissionRespData : listHrmLaidTypeRespData)
        }
        this.resetLoad()
      } catch (error) {
        this.outputError(error)
