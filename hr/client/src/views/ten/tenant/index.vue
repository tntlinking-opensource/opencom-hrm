...
import { listTenantPage, deleteTenant } from '@/api/ten/tenant'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [],
      tableId: '1373952341416665145',
      schemeId: '1373952341416665143'
    }
  },
  computed: {},
  methods: {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 租户ten_tenant
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listTenantRespData, listPermissionRespData] = await Promise.all([
          listTenantPage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listTenantRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listTenantRespData.data.total
          this.tableData = listTenantRespData.data.rows ? listTenantRespData.data.rows : []
          this.permission.view = listPermissionRespData.data.find((item) => {
            return item.permission === 'tenant:read'
          })
          this.permission.export = listPermissionRespData.data.find((item) => {
            return item.permission === 'tenant:export'
          })

          this.permission.add = listPermissionRespData.data.find((item) => {
            return item.permission === 'tenant:create'
          })
          this.permission.edit = listPermissionRespData.data.find((item) => {
            return item.permission === 'tenant:update'
          })
          this.permission.remove = listPermissionRespData.data.find((item) => {
            return item.permission === 'tenant:delete'
          })
        } else {
          this.showMessage(listPermissionRespData.code != 100 ? listPermissionRespData : listTenantRespData)
        }
        this.resetLoad()
      } catch (error) {
        this.outputError(error)
