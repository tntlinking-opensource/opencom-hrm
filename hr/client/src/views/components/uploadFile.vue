  ...
  export default {
    ...
    methods: {
      ...
      onViewFileList(index, row) { // 预览
        ...
        // 文件流访问路径
        let url = process.env.FILE_PREVIEW_URL + `sys/sysSeting/getSById/` + fileId + `?token=` + token
        window.open(process.env.KK_FILE_URL + encodeURIComponent(Base64.encode(url)));
        /*let fileId = row.objectId
        // 文件流访问路径
        let url = "http://377328t1x9.zicp.vip:52265/" + `sys/sysSeting/getFile/` + fileId
        console.log(url)
        console.log("打开的地址:","http://61.172.179.56:9898/insideOnlinePreview?url=" + encodeURIComponent(Base64.encode(url)))
        window.open("http://61.172.179.56:9898/insideOnlinePreview?url=" + encodeURIComponent(Base64.encode(url)));*/
      },
