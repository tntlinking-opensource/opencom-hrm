      ...
      <el-row>
        ...
        <el-form ref="editForm" :model="editFormData" label-width="100px" :disabled="action == 'view'"
          ...
          <el-row>
            ...
            <el-col :span="6"> <el-form-item prop="monthAvg" :rules="[
              { required: true, message: '月平均金额不能为空', trigger: 'blur' }
            ]" label="月平均金额" class="dc-el-form-item_SingleInput">
                <number-input v-model="editFormData.monthAvg" :maxLength="18" :precision="2" :min="0"
                  placeholder="请输入月平均金额" clearable autocomplete="new-password"
        ...
        <el-form ref="taxDetailsInfoForm" :model="editFormData" label-width="100px" :disabled="action == 'view'"
          ...
          <el-row>
            <ux-grid :data="editFormData.taxDetailsInfoList" ref="taxDetailsInfoTable" :currentRow="table124CurrentRow"
              ...
              <ux-table-column field="amountDown" title="年终奖月平均下限" tree-node resizable min-width="120px" align="left"
                header-align="center" show-overflow :edit-render="{ autofocus: '.el-input__inner' }"
                class="dc-ux-table-column_SingleInput">
                <template v-slot:header="{ column }">
                  <span style="color: #f56c6c">*</span>
                ...
                <template v-slot:edit="{ row, rowIndex }">
                  <el-form-item :prop="'taxDetailsInfoList.' + rowIndex + '.amountDown'" label-width="0px"
                    class="dc-el-form-item_SingleInput">
                    <number-input v-model="row.amountDown" :min="0" :precision="2" :maxLength="18"
              ...
              <ux-table-column field="amountUp" title="年终奖月平均上限" tree-node resizable min-width="120px" align="left"
                header-align="center" show-overflow :edit-render="{ autofocus: '.el-input__inner' }"
                class="dc-ux-table-column_SingleInput">
                <template v-slot:header="{ column }">
                  <span style="color: #f56c6c">*</span>
                ...
                <template v-slot:edit="{ row, rowIndex }">
                  <el-form-item :prop="'taxDetailsInfoList.' + rowIndex + '.amountUp'" label-width="0px"
                    class="dc-el-form-item_SingleInput">
                    <number-input v-model="row.amountUp" :maxLength="18" placeholder="请输入年终奖月平均上限" :precision="2" :min="0"
              ...
              <ux-table-column field="speedDeductAmount" title="速算扣除数" tree-node resizable min-width="120px" align="left"
                ...
                <template v-slot:edit="{ row, rowIndex }">
                  <el-form-item :prop="'taxDetailsInfoList.' + rowIndex + '.speedDeductAmount'" label-width="0px"
                    class="dc-el-form-item_SingleInput">
                    <number-input v-model="row.speedDeductAmount" :maxLength="18" placeholder="请输入数算扣除数" clearable
                      :precision="2" :min="0" autocomplete="new-password" class="dc-el-input_SingleInput"></number-input>
                  </el-form-item>
                </template>
                <template v-slot="{ row, rowIndex }">
                  <span class="my-input-sc">
    ...
    doSave() {
      if (this.editFormData.taxDetailsInfoList.length > 0) {
        ...
        arr.forEach(item => {
          ...
          console.log(item.speedDeductAmount, item.amountDown, item.amountUp, 'item.amountUp')
        ...
        if (!isEmpty && isValid) {
          ...
          if (isEmpty) {
            ...
            this.$message({
              type: 'warning',
              message: '请填写速算扣除数/年终奖月平均上限/年终奖月平均下限！'
            })
          }
        }
      } else {
