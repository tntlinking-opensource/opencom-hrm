...
import { listCompanyCodeRulePage, deleteCompanyCodeRule } from '@/api/sys/companyCodeRule'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [],
      tableId: '1331242413527212586',
      schemeId: '1331242413527212584'
    }
  },
  computed: {},
  methods: {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 公司编号规则sys_company_code_rule
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listCompanyCodeRuleRespData, listPermissionRespData] = await Promise.all([listCompanyCodeRulePage(this.search), listResourcePermission(this.$route.meta.routerId)])
        if (listCompanyCodeRuleRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listCompanyCodeRuleRespData.data.total
          this.tableData = listCompanyCodeRuleRespData.data.rows ? listCompanyCodeRuleRespData.data.rows : []
    ...
    onDelete(scope) {
      this.$confirm('确定删除吗？', '确认', {
        ...
        .then(() => {
          this.setLoad()
          deleteCompanyCodeRule(scope.row)
            .then((responseData) => {
              if (responseData.code == 100) {
                this.getList()
                this.showMessage({
