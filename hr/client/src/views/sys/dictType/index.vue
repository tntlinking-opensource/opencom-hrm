<template>
  <el-row class="dc-container" v-loading="loading">
    ...
    >
      <el-row>
        <el-col :span="6">
          <el-form-item prop="name" label="字典类型名" class="dc-el-form-item_SingleInput">
            <el-input
    ...
    >
      ...
      >
        ...
        <template slot-scope="scope">
          ...
          ></OperationIcon>
          <OperationIcon
            v-on:click="onExportExcel(scope.$index, scope.row)"
            type="primary"
            content="导出"
            placement="bottom"
            icon-name="el-icon-download"
...
import { listDictTypePage, deleteDictType, getDictTypeById, importDictType } from '@/api/sys/dictType'
    ...
    async pageInit() {
      ...
      try {
        ...
        if (listDictTypeRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listDictTypeRespData.data.total
          this.tableData = listDictTypeRespData.data.rows ? listDictTypeRespData.data.rows : []
