...
public class StarterGeneralManager implements ICandidate, ApplicationContextAware {
	...
	public List<ActUser> execute(VariableScope variableScope, String param) {
		...
		if(starter != null && starter.getDepartment() != null && StringUtils.isNotBlank(starter.getDepartment().getId())) {
			...
			if(company != null & company.getManager() != null && StringUtils.isNotBlank(company.getManager().getId()) ) {
				actUsers.add(new ActUser(company.getManager().getId(), company.getManager().getName()));
			}
		}
